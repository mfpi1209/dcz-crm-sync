...
`);o=o.replace(f,m)}),o}function ce(o){let e="";return o.analises_md&&(e+=`
  ...
  `,e}function me(o){console.log("=== FORMATANDO INFORMAÇÕES DE CURSOS ==="),console.log("Dados recebidos:",o);const _tryParseJson=(raw)=>{if(typeof raw!=="string")return null;let m=raw.trim();m=m.replace(/^```(?:json)?\s*/i,"").replace(/```\s*$/i,"").trim();if(m.toLowerCase().startsWith("json")&&!m.startsWith("{")&&!m.startsWith("["))m=m.substring(4).trim();if(!(m.startsWith("{")||m.startsWith("["))){const fb=m.match(/[\{\[][\s\S]*[\}\]]/);if(fb)m=fb[0];else return null}try{return JSON.parse(m)}catch(e1){try{const cleaned=m.replace(/,(\s*[}\]])/g,"$1");return JSON.parse(cleaned)}catch(e2){try{return new Function("return ("+m+")")()}catch(e3){console.warn("[_tryParseJson] tudo falhou:",e1.message,"|cleaned:",e2.message,"|eval:",e3.message,"\nPrimeiros 300 chars do input:",m.slice(0,300));return null}}}};if(typeof o==="string"){const p=_tryParseJson(o);if(p)o=p}if(o&&typeof o==="object"){for(const k of ["mensagem","resposta_para_usuario","output","response","message","result","data"]){const v=o[k];if(typeof v==="string"){const trimmed=v.trim();const looksLikeJson=trimmed.startsWith("{")||trimmed.startsWith("[")||trimmed.startsWith("```");if(looksLikeJson){const parsed=_tryParseJson(v);if(parsed&&typeof parsed==="object"){o=Object.assign({},o,parsed);delete o[k];console.log(`[me] Campo '${k}' parseado e mesclado`);break}else{console.warn(`[me] Campo '${k}' parece JSON mas falhou parse — suprimindo pra nao exibir cru`);delete o[k]}}}}}const e=t=>{if(!t)return"";const a=document.createElement("div");return a.textContent=t,a.innerHTML};let s="";o.resposta_para_usuario&&(s+=`
