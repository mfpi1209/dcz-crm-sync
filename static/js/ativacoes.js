...
function _ativRenderKPIs(curA, prevA, pctA, curR, prevR, pctR) {
    ...
    document.getElementById("ativ-kpi-ativ-icon").className   = `text-sm ${pctA > 0 ? "text-emerald-700 dark:text-green-400" : pctA < 0 ? "text-red-700 dark:text-red-400" : "text-slate-500"}`;
    ...
    document.getElementById("ativ-kpi-ret-icon").className   = `text-sm ${pctR > 0 ? "text-emerald-700 dark:text-green-400" : pctR < 0 ? "text-red-700 dark:text-red-400" : "text-slate-500"}`;
}
...
function _ativSetBadge(id, pct) {
    ...
    if (pct > 0)      el.className = "px-2 py-0.5 rounded-full font-semibold text-xs bg-emerald-100 dark:bg-green-500/20 text-emerald-800 dark:text-green-400";
    else if (pct < 0) el.className = "px-2 py-0.5 rounded-full font-semibold text-xs bg-red-100 dark:bg-red-500/20 text-red-800 dark:text-red-400";
    else               el.className = "px-2 py-0.5 rounded-full font-semibold text-xs bg-slate-200/80 dark:bg-slate-500/20 text-slate-600 dark:text-slate-400";
}
...
function _ativRenderTblAtivacoes(rows) {
    ...
    tb.innerHTML = rows.map(d =>
        `<tr class="hover:bg-slate-50 dark:hover:bg-white/[0.02]">
            <td class="px-5 py-2.5 text-slate-700 dark:text-slate-300">${esc(d.dia)}</td>
            <td class="px-5 py-2.5 text-right text-slate-900 dark:text-slate-200 font-semibold">${_ativFmt(d.total)}</td>
        </tr>`
    ).join("");
}
...
function _ativRenderTblRetornos(rows) {
    ...
    tb.innerHTML = rows.map(r =>
        `<tr class="hover:bg-slate-50 dark:hover:bg-white/[0.02]">
            <td class="px-5 py-2.5 text-[#00346f] dark:text-blue-400 font-medium">${esc(r.retorno.toUpperCase())}</td>
            <td class="px-5 py-2.5 text-right text-slate-900 dark:text-slate-200 font-semibold">${_ativFmt(r.quantidade)}</td>
        </tr>`
    ).join("");
}
